...
- **Minimum Node.js is `>=22.12`** (required by `oxc-parser`).
- **`peerDependencies.typescript` removed**: deps-finder never imports TypeScript.
- **Help text is generated by the CLI library**; the `--help` layout changed.

### Added

- `-v/--version`, an optional `<root>` project-directory argument, `--flag=value`, repeated `--ignore`/`--exclude`.
- `.mts` and `.cts` sources are analysed.
- A warning on stderr for each source file that cannot be read (its imports used to vanish silently).
- `tsconfig.json` is read as JSONC (comments, trailing commas), so its `outDir` is excluded.
- `package.json` with a UTF-8 byte order mark is accepted.
- Colour only on a TTY, and never with `NO_COLOR` set.
- Development files are scanned: tests, stories, `scripts/`, root `*.config.*` files and hidden directories. A package they use is not unused, but only production files make a devDependency misplaced (#56, #57).
- Workspaces: a member declared by `workspaces` or `pnpm-workspace.yaml`, or a directory with its own lockfile or `node_modules`, is a separate package. The root is credited when a member uses a package it hoists (#57).
- Type-level use counts: `@types/X` paired with imports of `X` (`@types/node` with Node built-ins), tsconfig `types`, `extends` and `importHelpers`, `/// <reference types>`, `import("x").T`, JSDoc imports, `declare module`, and hand-written `.d.ts` files (#58).
- The JSX runtime (`jsx`, `jsxImportSource`), nested tsconfigs and project references, test-runner globals and `verbatimModuleSyntax` are honoured. An import used only as a type is elided only where it would otherwise be misplaced (#59).
- Use without an import counts: a binary run from `scripts`, a declared package that satisfies a used package's peer, and a package named in a known tool's config (ESLint, Babel, PostCSS, Jest, Storybook, …) (#60).
- `.vue`, `.svelte` and `.astro` scripts, and `@import`/`@use` in CSS, SCSS and Less, count as use. Sources with a UTF-16 byte order mark are decoded (#61).
- `require.resolve`, `createRequire`, `import.meta.resolve`, template literals without expressions, `?query` suffixes, `package.json` `imports` (`#…`), tsconfig `paths`/`baseUrl` and `optionalDependencies` are understood (#62).
- Warnings on stderr for a directory that cannot be read, a stylesheet that does not parse, and a source the parser stopped in (#61, #62).

### Fixed

- Re-exports (`export * from`, `export { x } from`), literal dynamic `import()`, `$` bindings, `import x = require()`, and trailing-comma type imports are recognised.
- `/*` or `//` inside strings and comments no longer swallows the code that follows; import-like text inside template literals is no longer counted.
- A project under a folder named `test/`, `e2e/`, `build/`, … no longer has every file excluded.
- `--json` output larger than 64 KiB is no longer cut off when piped.
- A package declared in two sections is reported once; it is not misplaced when it is also in `dependencies` or is a peer kept in `devDependencies`.
- A declared npm polyfill (`buffer`, `events`, `process`) counts as used.
- A tsconfig `outDir: ""` no longer excludes every source file.
- A non-object dependency section (`"dependencies": "lodash"`) is a parse error instead of reporting packages `"0"`…`"5"`.
- "used in N files" counts files, not import lines; misplaced locations are sorted by file and line.
- Files are walked in path order, so output and exit codes are the same on every platform (#58).
- `.gitignore` is honoured, and gitignored directories are not walked at all (#57, #61).
- A deeply nested source no longer crashes the run: the analysis runs on a worker thread with a larger stack, and the AST is walked without recursion (#62).

### Changed

- Import parsing uses `oxc-parser`.
- `@mobily/ts-belt` and `ts-pattern` replaced by `effect` 4 (RC); lint and format by `oxlint` and `oxfmt`; build by TypeScript 7 `tsc`.
- Development pins Bun 1.4.2 through `packageManager` (#55); the unused `bench` script is gone (#64).
