...
Imports are now read by a real parser (oxc) instead of regular expressions, the CLI runs on Effect 4, and the exit code tells a failed run apart from a failed check.
...
- Import parsing uses `oxc-parser`. On a 2,015-file project the findings are unchanged and a run takes about 2.4 s instead of 1.4 s.
