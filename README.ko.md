...
6. import 없이 쓰는 패키지를 셉니다. 모두 development 사용으로 세므로 misplaced나 type-only가 되지는 않습니다.
   - **스크립트가 실행하는 바이너리.** 루트와 검사하는 각 레이아웃 루트의 `package.json` `scripts`, `.husky/`의 git 훅(확장자 없는 파일), lint-staged 명령을 `&&`, `||`, `;`, `|`, 줄바꿈으로 나눠 명령마다 봅니다. 앞의 `NAME=value` 대입과 실행기(`npx`, `npm exec`, `pnpm exec`, `pnpm dlx`, `yarn exec`, `yarn dlx`, `bunx`, `bun x`, `cross-env`, `dotenv … --`)는 건너뛰고, 그다음 낱말을 명령으로 봅니다. `npm run`, `pnpm`, `yarn`, `bun`(`run`이 있든 없든) 뒤의 낱말이 같은 `package.json`의 스크립트 이름이면 바이너리가 아니라 그 스크립트입니다. 명령은 설치된 `package.json`의 `bin`에 그 이름을 둔 선언 패키지를 사용합니다(문자열 `bin`은 스코프를 뺀 패키지 이름). 패키지는 Node처럼 프로젝트 루트와 그 위의 `node_modules`에서 찾습니다. 설치되지 않은 선언 패키지는 자기 이름, 그리고 스코프를 뺀 이름과 맞춥니다.
   - **peer.** 사용 중이고 설치된 의존성이 `peerDependencies`나 `peerDependenciesMeta`에 둔 선언 패키지는 optional이든 아니든 사용 중이며, 그 패키지가 둔 선언 peer도 이어서 사용 중입니다.
   - 프로젝트나 그 위에 `node_modules`가 전혀 없으면 바이너리는 이름으로 맞췄고 peer는 보지 않았다고 stderr에 한 번 알립니다.
   - **도구 설정.** 각 레이아웃 루트에서 아래 파일과 `package.json` 키를 도구마다의 짧은 이름 규칙으로 읽습니다(JSON·YAML은 데이터로, JS·TS는 실행하지 않고 객체 리터럴에서). 파일 안의 객체를 모두 보므로 `overrides`나 중첩된 블록도 셉니다.
     ...
     - Babel(`.babelrc*`, `babel.config.*`, `babel`): `presets`와 `plugins`(`@babel/env` → `@babel/preset-env`, `macros` → `babel-plugin-macros`, `module:x` → `x`).
     ...
     - 레이아웃 루트의 그 밖의 `*.config.*`·`*.preset.*` JS·TS 파일: `vite.config.ts`의 `minify: 'terser'`처럼 패키지 이름과 정확히 같은 문자열.
