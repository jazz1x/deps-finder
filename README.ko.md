...
6. import 없이 쓰는 패키지를 셉니다. 이것으로 misplaced나 type-only가 되는 패키지는 없습니다.
   ...
   - 프로젝트나 그 위의 `node_modules`에 설치된 선언 패키지가 하나도 없으면 바이너리는 이름으로 맞췄고 peer는 보지 않았다고 stderr에 한 번 알립니다.
   - **도구 설정.** 각 레이아웃 루트에서 아래 파일과 `package.json` 키를 도구마다의 짧은 이름 규칙으로 읽습니다(JSON·YAML은 데이터로, JS·TS는 실행하지 않고 객체 리터럴에서). `.eslintrc`처럼 확장자 없는 rc 파일은 주석을 허용하는 JSON으로 읽고, 안 되면 YAML로 읽습니다. 파일 안의 객체를 모두 보므로 `overrides`나 중첩된 블록도 셉니다. 여기서 레이아웃 루트에는 `project.json`만 있는 Nx 프로젝트도 들어갑니다.
