...
oxc 네이티브 파서가 재귀로 감당할 수 있는 것보다 깊게 중첩된 소스는 프로세스를 segmentation fault로 죽이고(종료 코드 `139`, 위 표 밖의 코드), 보고서는 나오지 않습니다. macOS arm64의 기본 8 MB 스택에서는 객체 리터럴 약 4,600단, 배열이나 괄호 약 5,500단이 한계이고, else-if 사슬과 문자열 이어 붙이기는 훨씬 깊이 갑니다(20,000단, 50,000항까지 파싱됨). 충돌은 파서 안에서 일어나 deps-finder가 잡을 수 없습니다. 스택 한도를 올리면 한계도 올라갑니다: `ulimit -s 65520` 아래에서는 20,000단 배열도 파싱됩니다. 이런 파일은 대개 생성물이니 `--exclude`로 빼세요.
