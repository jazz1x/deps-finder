...
6. Count the packages a project uses without importing them. None of these makes a package misplaced or type-only.
   ...
   - Unused is decided only against an install, as knip and depcheck do; lockfiles are not read. When packages are declared but none is installed in a `node_modules` at or above the project, binaries and peers are unknown, so a run that would decide unused (one that checks at least one declared package, after `--ignore`) prints one error naming the project directory, prints no report, and exits `2`. Install the dependencies first. A run that checks nothing for unused, such as the default run of a project with only `devDependencies`, goes on as usual.
