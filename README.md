...
2. Walk the project for `.ts`, `.tsx`, `.mts`, `.cts`, `.js`, `.jsx`, `.mjs`, `.cjs`, hidden files and directories included. The walk skips `.git/` and `node_modules/`, anything the project's `.gitignore` files ignore (the root one, nested ones, and those in parent directories up to the git repository's top, plus `.git/info/exclude`; git's rules, always matched case-sensitively, unlike git with `core.ignorecase` on, the macOS and Windows default), build outputs at each layout root (see below): `dist/`, `build/`, `out/` and `coverage/`, and the auto-detected output dirs at the project root (tsconfig `outDir`, `--outDir` in scripts, `*-dist`-style names). A project directory that its repository ignores is still scanned, with only its own `.gitignore` files. Symlinked files are followed; symlinked directories are not. A project without a `.gitignore` at its root (or above it in the repository) also skips common framework and cache dirs at each layout root (`.next/`, `.turbo/`, `.cache/`, `storybook-static/` and the like) and `.venv/`, `.gradle/`, `.claude/`, `.idea/` and `.vscode/` at any depth. A subdirectory whose `package.json` has a `name` and declares dependencies (a non-empty `dependencies`, `devDependencies`, `peerDependencies` or `optionalDependencies`) is a separate package, so its whole tree is left out and stderr names it. A named `package.json` that declares no dependencies, such as an Nx-style lib whose dependencies live in the root `package.json`, stays part of this run, the way Node resolution falls back to the root. A lib that declares some dependencies but also imports ones only the root declares is still left out, so those root dependencies read as unused here and its own run does not check them either. The project root and every scanned directory whose `package.json` has a `name` are layout roots; a file is matched against every layout root above it, not only the nearest. A `package.json` without a `name`, such as `{"sideEffects": false}`, is neither a separate package nor a layout root. Neither is a directory that has only an Nx `project.json`, so its config files, such as `vite.config.ts` or `playwright.config.ts`, are production; give it a `package.json` with a `name` and no dependencies to make it a layout root. Each file is tagged **development** or **production**. Development files are tests, specs, stories, and test setup files; anything under `test/`, `tests/`, `__tests__/`, `__mocks__/`, `e2e/`, `cypress/`, `playwright/` or `.storybook/` at any depth; dotfiles such as `.eslintrc.js` at any depth; and, only at a layout root, `*.config.*` and `*.preset.*` files (`webpack.config.prod.js` included), the `scripts/` directory and hidden directories such as `.husky/` or `.github/`. Everything else is production, including `src/app.config.ts`, `src/scripts/`, `src/.generated/` and a feature folder named `stories/`.
